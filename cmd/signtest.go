...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Hash Struct Typed Data
		/* typedDataHash, err := signerData.HashStruct("signature", signerData.Message)
		if err != nil {
			log.Fatal(err)
		}
		fmt.Println("signat typed:", typedDataHash)

		// Hash Struct for Domain
		domainSeparator, err := signerData.HashStruct("EIP712Domain", signerData.Domain.Map())
		if err != nil {
			log.Fatal(err)
		}
		fmt.Println("domain:", domainSeparator) */

		uhuhuh, err := signerv4.HashTypedDataV4(signerData, big.NewInt(4))
		if err != nil {
			log.Fatal(err)
		}

		fmt.Println("UH:", uhuhuh)

		hope, err := signerv4.SignTypedDataV4(signerData, privateKey, big.NewInt(4))
		if err != nil {
			log.Fatal(err)
		}

		// Yeah that's a valid friggin signature, fricker
		fmt.Println("hope:", hope)

		/* uh2, err := signerv4.ValidateAndHash(signerData, big.NewInt(4)) */
